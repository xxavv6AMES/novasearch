...
import { performSearch, createSearchResult, createPagination } from './services/search.js';
...
async function handleSearch(query, startIndex = 1) {
    const resultsContainer = document.querySelector('.results-container');
    
    try {
        resultsContainer.innerHTML = '<div class="loading">Searching...</div>';
        
        const results = await performSearch(query, startIndex);
        ...
        document.querySelectorAll('.page-btn').forEach(button => {
            button.addEventListener('click', (e) => {
                const startIndex = e.currentTarget.dataset.start;
                handleSearch(query, startIndex);
                window.scrollTo(0, 0);
            });
        });
    } catch (error) {
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Handle search if we're on results page
    if (!isHomePage) {
